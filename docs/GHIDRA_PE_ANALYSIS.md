...
| Ssd_Parser.dll | 357 | — | 0 | `ghidra_output_parser/all_decompiled.c` |
| CnvMA3SMF.dll | 495 | — | 0 | `ghidra_output_cnvma3smf/all_decompiled.c` |
| CnvMA3SMAF_SC.dll | 876 | — | 0 | `ghidra_output_cnvma3smaf_sc/all_decompiled.c` |
...
| 版本 | 策略 | 结果 |
|------|------|------|
| v1~v5 | 通过 SSD.dll → sscma3.dll 完整链路 | 各种错误（崩溃、buffer too small 等） |
| v6 | 通过 sscma3.dll 调用 CnvMA3SMF.dll | 0x31000101（buffer 太小） |
| v7~v8 | VEH 异常处理绕过 Ssd_Parser 崩溃 | VEH 无限循环 |
| v9 | 直接调用 CnvMA3SMF.dll::CnvTo | 成功，输出处理后的 SMF |
| **v10** | **两阶段：CnvMA3SMF → CnvMA3SMAF_SC** | **成功，输出完整 MMF 文件** |

**v10 最终流程：**
```
SMF (MIDI) → CnvMA3SMF.dll::CnvTo (ordinal 2) → 处理后 SMF
           → CnvMA3SMAF_SC.dll::CnvTo (ordinal 3) → MMF (MMMD格式)
```

**v10 已验证：**
- 输出 MMMD 格式 MMF 文件，可正常播放
- 音色为 Standard 风格（CnvMA3SMAF_SC.dll 内置 s_ToolDefaultVoice）
- BrilliantSnow.mid (10905B) → 5673B MMF
- Sony PS2 Seq.mid (5156B) → 3543B MMF
...
- **已验证：内置默认音色 = Standard 风格，输出的 MMF 可正常播放**
