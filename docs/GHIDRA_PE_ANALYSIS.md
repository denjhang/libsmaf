...
**this+0xD0 缓冲区验证（param_5 来自 Rsrc）：**
```c
if (buffer[0] != 0)         return 0xF1F00000;  // byte[0] 必须为 0
if (buffer[1] > 2 && buffer[1] != 0x23) || buffer[2] < 0xF8)
                            return 0xF1F00000;  // byte[1]<=2 或 0x23, byte[2]>=0xF8
```
...
| 2. 资源初始化 | Ssd_SscCnv_Rsrc | 返回 0 | 但参数可能不正确 |
| 3. SMF0→SMAF | Ssd_SscCnv | 失败 -5 | 错误码 0xF1F00000 |

**根本原因分析：** 当前 midi2mmf.c 调用链不完整。SSD.exe 的完整流程需要先通过路径 A（Ssd_SmfParse_Rsrc 解析 SMF 数据，生成语音信息和流配置），然后路径 B 才能工作。直接跳过解析步骤调用 Ssd_SscCnv_Rsrc/Ssd_SscCnv 会导致参数不完整，验证失败。

**待解决：** 需要补充 Ssd_SmfParse_Rsrc 步骤，或找到绕过验证的方法。可能需要反编译 Ssd_Parser.dll 来理解解析步骤的输入输出格式。
