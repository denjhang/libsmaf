package com.google.android.gms.common.api;

import android.accounts.Account;
import android.content.Context;
import android.os.IBinder;
import android.os.IInterface;
import android.os.Looper;
import com.google.android.gms.auth.api.signin.GoogleSignInAccount;
import com.google.android.gms.common.Feature;
import com.google.android.gms.common.api.a.d;
import com.google.android.gms.common.internal.AbstractC0065c;
import com.google.android.gms.common.internal.C0066d;
import com.google.android.gms.common.internal.InterfaceC0073k;
import com.google.android.gms.common.internal.r;
import java.util.Set;

/* JADX INFO: loaded from: D:\working\vscode-projects\Reference_Project\RP2350-Reference\smaf_20260516\apk_extract\classes.dex */
public final class a<O extends d> {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    private final AbstractC0021a<?, O> f1030a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    private final i<?, O> f1031b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    private final g<?> f1032c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    private final j<?> f1033d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    private final String f1034e;

    /* JADX INFO: renamed from: com.google.android.gms.common.api.a$a, reason: collision with other inner class name */
    public static abstract class AbstractC0021a<T extends f, O> extends e<T, O> {
        public abstract T a(Context context, Looper looper, C0066d c0066d, O o, com.google.android.gms.common.api.f fVar, com.google.android.gms.common.api.g gVar);
    }

    public interface b {
    }

    public static class c<C extends b> {
    }

    public interface d {

        /* JADX INFO: renamed from: com.google.android.gms.common.api.a$d$a, reason: collision with other inner class name */
        public interface InterfaceC0022a extends c, InterfaceC0023d {
            Account a();
        }

        public interface b extends c {
            GoogleSignInAccount b();
        }

        public interface c extends d {
        }

        /* JADX INFO: renamed from: com.google.android.gms.common.api.a$d$d, reason: collision with other inner class name */
        public interface InterfaceC0023d extends d {
        }

        public interface e extends c, InterfaceC0023d {
        }
    }

    public static abstract class e<T extends b, O> {
    }

    public interface f extends b {
        void a();

        void a(AbstractC0065c.InterfaceC0026c interfaceC0026c);

        void a(AbstractC0065c.e eVar);

        void a(InterfaceC0073k interfaceC0073k, Set<Scope> set);

        boolean b();

        String c();

        boolean d();

        int e();

        Feature[] f();

        boolean g();

        boolean isConnected();
    }

    public static final class g<C extends f> extends c<C> {
    }

    public interface h<T extends IInterface> extends b {
        T a(IBinder iBinder);

        void a(int i, T t);

        String h();

        String i();
    }

    public static abstract class i<T extends h, O> extends e<T, O> {
    }

    public static final class j<C extends h> extends c<C> {
    }

    /* JADX WARN: Multi-variable type inference failed */
    public <C extends f> a(String str, AbstractC0021a<C, O> abstractC0021a, g<C> gVar) {
        r.a(abstractC0021a, "Cannot construct an Api with a null ClientBuilder");
        r.a(gVar, "Cannot construct an Api with a null ClientKey");
        this.f1034e = str;
        this.f1030a = abstractC0021a;
        this.f1031b = null;
        this.f1032c = gVar;
        this.f1033d = null;
    }

    public final String a() {
        return this.f1034e;
    }

    public final AbstractC0021a<?, O> b() {
        r.b(this.f1030a != null, "This API was constructed with a SimpleClientBuilder. Use getSimpleClientBuilder");
        return this.f1030a;
    }
}
