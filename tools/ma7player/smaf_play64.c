...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        printf("Usage: %s <input.mmf> [-r samplerate] [-dll path] [-ma3/-ma5/-ma7]\n", argv[0]);
    ...
    printf("Loaded MMF: %d bytes (64-bit player)\n", mmf_size);
    ...
    // Load MMF
    int sound_id = ma_load_64(&ctx, FORMAT_MMF, mmf_data, mmf_size);
    if (sound_id < 0) {
    ...
    // Get length
    int length_ms = ma_get_length_64(&ctx, FORMAT_MMF, sound_id);
    printf("Length: %d ms (%.2f seconds)\n", length_ms, length_ms / 1000.0);

    // Start playback
    if (ma_start_64(&ctx, FORMAT_MMF, sound_id, 0) != 0) {
    ...
    while (elapsed < length_ms + 1000) {
        int status = ma_get_status_64(&ctx, FORMAT_MMF, sound_id);
